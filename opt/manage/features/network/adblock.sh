...
adblock_config_get() {
  need_python3
  [[ -f "${ADBLOCK_CONFIG_FILE}" ]] || {
    printf 'enabled=0\ndns_port=-\nupstream_primary=1.1.1.1\nupstream_secondary=8.8.8.8\nauto_update_enabled=0\n'
    return 0
  }
  python3 - <<'PY' "${ADBLOCK_CONFIG_FILE}" 2>/dev/null || true
import pathlib
import sys
...
print(f"enabled={data.get('AUTOSCRIPT_ADBLOCK_ENABLED', '0')}")
print(f"dns_port={data.get('AUTOSCRIPT_ADBLOCK_PORT') or data.get('AUTOSCRIPT_ADBLOCK_PORT', '-')}")
print(f"upstream_primary={data.get('AUTOSCRIPT_ADBLOCK_UPSTREAM_PRIMARY') or data.get('AUTOSCRIPT_ADBLOCK_UPSTREAM_PRIMARY', '1.1.1.1')}")
print(f"upstream_secondary={data.get('AUTOSCRIPT_ADBLOCK_UPSTREAM_SECONDARY') or data.get('AUTOSCRIPT_ADBLOCK_UPSTREAM_SECONDARY', '8.8.8.8')}")
...
adblock_dns_resolver_runtime_conf_write() {
  ...
  cfg="$(adblock_dns_resolver_config_get)"
  dns_port="$(printf '%s\n' "${cfg}" | awk -F'=' '/^dns_port=/{print $2; exit}')"
  primary="$(printf '%s\n' "${cfg}" | awk -F'=' '/^upstream_primary=/{print $2; exit}')"
  secondary="$(printf '%s\n' "${cfg}" | awk -F'=' '/^upstream_secondary=/{print $2; exit}')"
  [[ "${dns_port}" =~ ^[0-9]+$ ]] || dns_port="5353"
  ...
  mkdir -p "$(dirname "${ADBLOCK_DNSMASQ_CONF}")" 2>/dev/null || true
  tmp="$(mktemp "${WORK_DIR}/.adblock-dnsmasq.XXXXXX" 2>/dev/null || true)"
  [[ -n "${tmp}" ]] || tmp="${WORK_DIR}/.adblock-dnsmasq.$$"
  cat > "${tmp}" <<EOF
# DNS Adblock resolver
port=${dns_port}
...
server=${primary}
server=${secondary}
conf-file=${ADBLOCK_RENDERED_FILE}
EOF
  mv -f "${tmp}" "${ADBLOCK_DNSMASQ_CONF}" || {
    rm -f "${tmp}" >/dev/null 2>&1 || true
    return 1
  }
  chmod 644 "${ADBLOCK_DNSMASQ_CONF}" >/dev/null 2>&1 || true
  return 0
}

adblock_dns_resolver_apply_now() {
  if [[ "${ADBLOCK_LOCK_HELD:-0}" != "1" ]]; then
    adblock_run_locked adblock_dns_resolver_apply_now "$@"
    return $?
  fi
  adblock_dns_resolver_runtime_conf_write || return 1
  if systemctl is-active --quiet "${ADBLOCK_DNS_SERVICE}"; then
    svc_restart_checked "${ADBLOCK_DNS_SERVICE}" 20 >/dev/null 2>&1 || {
      warn "Restart ${ADBLOCK_DNS_SERVICE} gagal."
      return 1
    }
  else
    svc_start_checked "${ADBLOCK_DNS_SERVICE}" 20 >/dev/null 2>&1 || {
      warn "Service ${ADBLOCK_DNS_SERVICE} gagal diaktifkan."
      return 1
    }
  fi
  adblock_apply_now
...
adblock_dns_resolver_set_upstreams() {
  if [[ "${ADBLOCK_LOCK_HELD:-0}" != "1" ]]; then
    adblock_run_locked adblock_dns_resolver_set_upstreams "$@"
    return $?
  fi
  local primary="${1:-}"
  local secondary="${2:-}"
  ...
  adblock_config_mutate_with_runtime \
    adblock_dns_resolver_apply_now \
    adblock_config_set_values \
    "Perubahan upstream DNS Adblock" \
    AUTOSCRIPT_ADBLOCK_UPSTREAM_PRIMARY "${primary}" \
    AUTOSCRIPT_ADBLOCK_UPSTREAM_SECONDARY "${secondary}"
}

adblock_config_set_enabled() {
  if [[ "${ADBLOCK_LOCK_HELD:-0}" != "1" ]]; then
    adblock_run_locked adblock_config_set_enabled "$@"
    return $?
  fi
  local value="${1:-0}"
  need_python3
  ...
  local tmp
  mkdir -p "$(dirname "${ADBLOCK_CONFIG_FILE}")" 2>/dev/null || true
  tmp="$(mktemp "${WORK_DIR}/.adblock-config.XXXXXX" 2>/dev/null || true)"
  [[ -n "${tmp}" ]] || tmp="${WORK_DIR}/.adblock-config.$$"
  python3 - <<'PY' "${ADBLOCK_CONFIG_FILE}" "${tmp}" "${value}"
import pathlib
import sys
...
for line in lines:
  stripped = line.strip()
  if stripped.startswith("AUTOSCRIPT_ADBLOCK_ENABLED="):
    out.append(f"AUTOSCRIPT_ADBLOCK_ENABLED={value}")
    found = True
  else:
    out.append(line)
if not found:
...
PY
  local rc=$?
  if (( rc == 0 )); then
    mv -f "${tmp}" "${ADBLOCK_CONFIG_FILE}" || {
      rm -f "${tmp}" >/dev/null 2>&1 || true
      return 1
    }
    chmod 644 "${ADBLOCK_CONFIG_FILE}" >/dev/null 2>&1 || true
  else
    rm -f "${tmp}" >/dev/null 2>&1 || true
  fi
  return "${rc}"
...
adblock_config_set_values() {
  ...
  local tmp
  mkdir -p "$(dirname "${ADBLOCK_CONFIG_FILE}")" 2>/dev/null || true
  touch "${ADBLOCK_CONFIG_FILE}"
  tmp="$(mktemp "${WORK_DIR}/.adblock-config.XXXXXX" 2>/dev/null || true)"
  [[ -n "${tmp}" ]] || tmp="${WORK_DIR}/.adblock-config.$$"

  python3 - <<'PY' "${ADBLOCK_CONFIG_FILE}" "${tmp}" "$@"
import pathlib
import sys
...
adblock_status_get() {
  ...
  raw_status="$("${ADBLOCK_SYNC_BIN}" --status 2>/dev/null || true)"
...
adblock_apply_now() {
  ...
  "${ADBLOCK_SYNC_BIN}" --apply >/dev/null 2>&1
}

adblock_config_mutate_with_runtime() {
  if [[ "${ADBLOCK_LOCK_HELD:-0}" != "1" ]]; then
    adblock_run_locked adblock_config_mutate_with_runtime "$@"
    return $?
  fi
  local apply_fn="${1:-}" mutate_fn="${2:-}" context="${3:-Perubahan DNS Adblock}"
  ...
  if [[ -f "${ADBLOCK_CONFIG_FILE}" ]]; then
    had_existing="1"
    backup="$(mktemp "${WORK_DIR}/.adblock-config.rollback.XXXXXX" 2>/dev/null || true)"
    [[ -n "${backup}" ]] || backup="${WORK_DIR}/.adblock-config.rollback.$$"
    if ! cp -a "${ADBLOCK_CONFIG_FILE}" "${backup}" 2>/dev/null; then
      rm -f "${backup}" >/dev/null 2>&1 || true
      warn "Gagal membuat backup config DNS Adblock sebelum apply."
      return 1
    fi
  fi
  ...
  if [[ "${had_existing}" == "1" ]]; then
    if ! cp -a "${backup}" "${ADBLOCK_CONFIG_FILE}" 2>/dev/null; then
      rollback_notes+=("restore config gagal")
    fi
  else
    rm -f "${ADBLOCK_CONFIG_FILE}" >/dev/null 2>&1 || rollback_notes+=("hapus config baru gagal")
  fi
...
adblock_set_enabled_now() {
  if [[ "${ADBLOCK_LOCK_HELD:-0}" != "1" ]]; then
    adblock_run_locked adblock_set_enabled_now "$@"
    return $?
  fi
  local value="${1:-0}"
  [[ "${value}" == "0" || "${value}" == "1" ]] || return 1
...
adblock_update_now() {
  ...
  "${ADBLOCK_SYNC_BIN}" "${args[@]}" >/dev/null 2>&1
}
...
adblock_auto_update_set_days() {
  ...
  previous_days="$(adblock_config_get | awk -F'=' '/^auto_update_days=/{print $2; exit}')"
...
adblock_manual_domains_list() {
  need_python3
  [[ -f "${ADBLOCK_BLOCKLIST_FILE}" ]] || return 0
  python3 - <<'PY' "${ADBLOCK_BLOCKLIST_FILE}" 2>/dev/null || true
import pathlib
import sys
...
adblock_manual_domain_add_commit() {
  ...
  mkdir -p "$(dirname "${ADBLOCK_BLOCKLIST_FILE}")" 2>/dev/null || true
  touch "${ADBLOCK_BLOCKLIST_FILE}"
  ...
  mkdir -p "${snap_dir}" 2>/dev/null || true
  snapshot_file_capture "${ADBLOCK_BLOCKLIST_FILE}" "${snap_dir}" "blocklist" || {
    warn "Gagal membuat snapshot blocklist sebelum update."
    rm -rf "${snap_dir}" >/dev/null 2>&1 || true
    return 1
  }
  tmp="$(mktemp "${WORK_DIR}/.adblock-blocklist.XXXXXX" 2>/dev/null || true)"
  [[ -n "${tmp}" ]] || tmp="${WORK_DIR}/.adblock-blocklist.$$"
  if [[ -f "${ADBLOCK_BLOCKLIST_FILE}" ]]; then
    cat "${ADBLOCK_BLOCKLIST_FILE}" > "${tmp}" || {
      rm -f "${tmp}" >/dev/null 2>&1 || true
      rm -rf "${snap_dir}" >/dev/null 2>&1 || true
  ...
  if ! mv -f "${tmp}" "${ADBLOCK_BLOCKLIST_FILE}"; then
    rm -f "${tmp}" >/dev/null 2>&1 || true
    rm -rf "${snap_dir}" >/dev/null 2>&1 || true
    warn "Gagal mengganti blocklist live."
    return 1
  fi
  chmod 644 "${ADBLOCK_BLOCKLIST_FILE}" >/dev/null 2>&1 || true
  if adblock_mark_dirty; then
    log "Domain Adblock ditambahkan. Jalankan Update Adblock untuk build artifact baru."
    rm -rf "${snap_dir}" >/dev/null 2>&1 || true
    return 0
  fi
  if snapshot_file_restore "${ADBLOCK_BLOCKLIST_FILE}" "${snap_dir}" "blocklist" >/dev/null 2>&1; then
...
adblock_manual_domain_add_menu() {
  ...
  while true; do
    ...
    if adblock_run_locked adblock_source_change_commit_and_sync "${ADBLOCK_BLOCKLIST_FILE}" "blocklist" "Domain manual Adblock berhasil ditambahkan." adblock_manual_domain_add_commit "${normalized}"; then
      pause
      return 0
    fi
    pause
...
adblock_manual_domain_delete_commit() {
  ...
  tmp="$(mktemp "${WORK_DIR}/.adblock-domains.XXXXXX" 2>/dev/null || true)"
  [[ -n "${tmp}" ]] || tmp="${WORK_DIR}/.adblock-domains.$$"
  : > "${tmp}"
  for i in "${!domains[@]}"; do
  ...
  mv -f "${tmp}" "${ADBLOCK_BLOCKLIST_FILE}" || {
    rm -f "${tmp}" >/dev/null 2>&1 || true
    rm -rf "${snap_dir}" >/dev/null 2>&1 || true
    warn "Gagal menulis blocklist baru."
    return 1
...
adblock_manual_domain_delete_menu() {
  ...
  while true; do
    ...
    if adblock_run_locked adblock_source_change_commit_and_sync "${ADBLOCK_BLOCKLIST_FILE}" "blocklist" "Domain manual Adblock berhasil dihapus." adblock_manual_domain_delete_commit "${selected_domain}"; then
      pause
      return 0
    fi
    pause
...
adblock_restore_runtime_state_checked() {
  # args: want_xray_enabled(0/1) want_dns_enabled(0/1)
  local want_xray="${1:-0}" want_dns="${2:-0}"
  ...
  if ! adblock_config_set_enabled "${want_dns}" >/dev/null 2>&1; then
    notes+=("rollback config DNS gagal")
  elif ! adblock_apply_now >/dev/null 2>&1; then
    notes+=("rollback apply DNS gagal")
  fi
...
adblock_action_preview_print() {
  ...
  local xray_enabled dns_enabled xray_outbound update_mode="none"
  ...
  echo "DNS rule     : $([[ "${dns_enabled}" == "1" ]] && echo "ON" || echo "OFF")"
  case "${action}" in
    enable)
      echo "Efek         : build/update artifact bila perlu, aktifkan routing Xray, apply DNS Adblock."
      echo "Mode update  : ${update_mode}"
      ;;
    disable)
      echo "Efek         : nonaktifkan routing Xray dan DNS Adblock sekarang."
      ;;
    update)
...
adblock_enable_all() {
  ...
  if ! adblock_config_set_enabled 1; then
    ...
    warn "Gagal mengaktifkan DNS Adblock."
    return 1
  fi

  if ! adblock_apply_now; then
    if ! rollback_msg="$(adblock_restore_runtime_state_checked "${xray_was_enabled}" "0")"; then
      die "Rollback enable Adblock gagal: ${rollback_msg}"
    fi
    warn "DNS Adblock gagal diterapkan."
    return 1
  fi

  log "Adblock diaktifkan (shared source -> Xray + DNS)."
  return 0
}
...
adblock_disable_all() {
  ...
  local dns_status="ON"
  local previous_xray previous_dns rollback_msg=""
  previous_xray="$(xray_routing_adblock_rule_get 2>/dev/null | awk -F'=' '/^enabled=/{print $2; exit}')"
  previous_dns="$(adblock_status_get | awk -F'=' '/^enabled=/{print $2; exit}')"
  ...
  if ! adblock_config_set_enabled 0; then
    warn "Gagal menonaktifkan DNS Adblock."
  elif adblock_apply_now; then
    dns_status="OFF"
  else
    warn "DNS Adblock gagal disinkronkan saat disable."
  fi

  if [[ "${xray_status}" == "OFF" && "${dns_status}" == "OFF" ]]; then
    log "Adblock dinonaktifkan (Xray + DNS)."
    return 0
  fi

  if ! rollback_msg="$(adblock_restore_runtime_state_checked "${previous_xray}" "${previous_dns}")"; then
    die "Rollback disable Adblock gagal: ${rollback_msg}"
  fi
  warn "Adblock nonaktif sebagian. Xray=${xray_status}, DNS=${dns_status}."
  return 1
}

adblock_show_bound_users() {
  title
  ...
  local rows
  rows="$("${ADBLOCK_SYNC_BIN}" --show-users 2>/dev/null || true)"
  if [[ -z "${rows//[[:space:]]/}" ]]; then
    echo "Belum ada user yang terikat ke DNS Adblock."
    hr
    pause
    return 0
  fi
...
adblock_dns_menu() {
  while true; do
    ...
    title
    echo "$(adblock_menu_title "DNS Adblock")"
    hr
    ...
    case "${c}" in
      1)
        if adblock_set_enabled_now 1; then
          log "DNS Adblock diaktifkan."
        else
          warn "DNS Adblock gagal diaktifkan."
        fi
        pause
        ;;
      2)
        if adblock_set_enabled_now 0; then
          log "DNS Adblock dinonaktifkan."
        else
          warn "DNS Adblock gagal dinonaktifkan."
        fi
        pause
        ;;
      3)
        adblock_url_add_menu
        ;;
      4)
        adblock_url_delete_menu
        ;;
      5)
        if adblock_apply_now; then
          log "DNS Adblock URL sources diperbarui."
        else
          warn "DNS Adblock URL sources gagal diperbarui."
        fi
        pause
        ;;
      6)
...
adblock_urls_list() {
  [[ -f "${ADBLOCK_URLS_FILE}" ]] || return 0
  grep -E '^[[:space:]]*https?://' "${ADBLOCK_URLS_FILE}" 2>/dev/null | sed 's/^[[:space:]]*//'
}

adblock_url_normalize() {
  local url="${1:-}"
...
adblock_url_add_commit() {
  local normalized="${1:-}"
  local snap_dir tmp
  [[ -n "${normalized}" ]] || return 1
  mkdir -p "$(dirname "${ADBLOCK_URLS_FILE}")" 2>/dev/null || true
  touch "${ADBLOCK_URLS_FILE}"
  if adblock_urls_list | grep -Fxq "${normalized}"; then
    warn "URL sudah ada."
    return 1
  fi
  ...
  mkdir -p "${snap_dir}" 2>/dev/null || true
  snapshot_file_capture "${ADBLOCK_URLS_FILE}" "${snap_dir}" "urls" || {
    warn "Gagal membuat snapshot URL source sebelum update."
    rm -rf "${snap_dir}" >/dev/null 2>&1 || true
    return 1
  }
  tmp="$(mktemp "${WORK_DIR}/.adblock-urls.XXXXXX" 2>/dev/null || true)"
  [[ -n "${tmp}" ]] || tmp="${WORK_DIR}/.adblock-urls.$$"
  if [[ -f "${ADBLOCK_URLS_FILE}" ]]; then
    cat "${ADBLOCK_URLS_FILE}" > "${tmp}" || {
      rm -f "${tmp}" >/dev/null 2>&1 || true
      rm -rf "${snap_dir}" >/dev/null 2>&1 || true
  ...
  if ! mv -f "${tmp}" "${ADBLOCK_URLS_FILE}"; then
    rm -f "${tmp}" >/dev/null 2>&1 || true
    rm -rf "${snap_dir}" >/dev/null 2>&1 || true
    warn "Gagal mengganti file URL source live."
    return 1
  fi
  chmod 644 "${ADBLOCK_URLS_FILE}" >/dev/null 2>&1 || true
  if adblock_mark_dirty; then
    log "URL source Adblock ditambahkan. Jalankan Update Adblock untuk build artifact baru."
    rm -rf "${snap_dir}" >/dev/null 2>&1 || true
    return 0
  fi
  if snapshot_file_restore "${ADBLOCK_URLS_FILE}" "${snap_dir}" "urls" >/dev/null 2>&1; then
...
adblock_url_add_menu() {
  ...
  while true; do
    ...
    normalized="$(adblock_url_normalize "${input}")" || {
    ...
    if adblock_run_locked adblock_source_change_commit_and_sync "${ADBLOCK_URLS_FILE}" "urls" "URL source Adblock berhasil ditambahkan." adblock_url_add_commit "${normalized}"; then
      pause
      return 0
    fi
    pause
...
adblock_url_delete_menu() {
  ...
  while true; do
    ...
    if adblock_run_locked adblock_source_change_commit_and_sync "${ADBLOCK_URLS_FILE}" "urls" "URL source Adblock berhasil dihapus." adblock_url_delete_commit "${selected_url}"; then
      pause
      return 0
    fi
    pause
...
adblock_menu() {
  ...
  while true; do
    ...
    local adblock_st dns_enabled dns_port dns_service dns_state sync_service sync_state nft_table users_count entries source_urls
    ...
    adblock_st="$(adblock_status_get)"
    dns_enabled="$(printf '%s\n' "${adblock_st}" | awk -F'=' '/^enabled=/{print $2; exit}')"
    dns_port="$(printf '%s\n' "${adblock_st}" | awk -F'=' '/^dns_port=/{print $2; exit}')"
    dns_service="$(printf '%s\n' "${adblock_st}" | awk -F'=' '/^dns_service=/{print $2; exit}')"
    dns_state="$(printf '%s\n' "${adblock_st}" | awk -F'=' '/^dns_service_state=/{print $2; exit}')"
    sync_service="$(printf '%s\n' "${adblock_st}" | awk -F'=' '/^sync_service=/{print $2; exit}')"
    sync_state="$(printf '%s\n' "${adblock_st}" | awk -F'=' '/^sync_service_state=/{print $2; exit}')"
    nft_table="$(printf '%s\n' "${adblock_st}" | awk -F'=' '/^nft_table=/{print $2; exit}')"
    users_count="$(printf '%s\n' "${adblock_st}" | awk -F'=' '/^users_count=/{print $2; exit}')"
    entries="$(printf '%s\n' "${adblock_st}" | awk -F'=' '/^blocklist_entries=/{print $2; exit}')"
    source_urls="$(printf '%s\n' "${adblock_st}" | awk -F'=' '/^source_urls=/{print $2; exit}')"
    dirty="$(printf '%s\n' "${adblock_st}" | awk -F'=' '/^dirty=/{print $2; exit}')"
    manual_domains="$(printf '%s\n' "${adblock_st}" | awk -F'=' '/^manual_domains=/{print $2; exit}')"
    merged_domains="$(printf '%s\n' "${adblock_st}" | awk -F'=' '/^merged_domains=/{print $2; exit}')"
    rendered_status="$(printf '%s\n' "${adblock_st}" | awk -F'=' '/^rendered_file=/{print $2; exit}')"
    xray_asset="$(printf '%s\n' "${adblock_st}" | awk -F'=' '/^custom_dat=/{print $2; exit}')"
    auto_update_enabled="$(printf '%s\n' "${adblock_st}" | awk -F'=' '/^auto_update_enabled=/{print $2; exit}')"
    auto_update_service="$(printf '%s\n' "${adblock_st}" | awk -F'=' '/^auto_update_service=/{print $2; exit}')"
    auto_update_service_state="$(printf '%s\n' "${adblock_st}" | awk -F'=' '/^auto_update_service_state=/{print $2; exit}')"
    auto_update_timer="$(printf '%s\n' "${adblock_st}" | awk -F'=' '/^auto_update_timer=/{print $2; exit}')"
    auto_update_days="$(printf '%s\n' "${adblock_st}" | awk -F'=' '/^auto_update_days=/{print $2; exit}')"
    auto_update_schedule="$(printf '%s\n' "${adblock_st}" | awk -F'=' '/^auto_update_schedule=/{print $2; exit}')"
    last_update="$(printf '%s\n' "${adblock_st}" | awk -F'=' '/^last_update=/{sub(/^last_update=/,""); print; exit}')"
    if [[ -z "${dns_state}" ]]; then
      if [[ -n "${dns_service}" && "${dns_service}" != "missing" ]] && svc_exists "${dns_service}"; then
        dns_state="$(svc_state "${dns_service}")"
      else
    ...
    if [[ "${xray_enabled}" == "1" && "${dns_enabled}" == "1" ]]; then
      overall_status="ON"
    elif [[ "${xray_enabled}" == "1" || "${dns_enabled}" == "1" ]]; then
    ...
    hr
    echo "Satu source: manual domains + URL sources. Output runtime: Xray custom.dat + DNS dnsmasq."
    hr
    printf "Status       : %s\n" "${overall_status}"
    ...
    hr
    printf "DNS Rule     : %s\n" "$([[ "${dns_enabled}" == "1" ]] && echo "ON" || echo "OFF")"
    printf "DNS Service  : %s\n" "${dns_service:--}"
    printf "DNS State    : %s\n" "${dns_state:--}"
    printf "Sync Service : %s\n" "${sync_service:--}"
    printf "Sync State   : %s\n" "${sync_state:--}"
    ...
    case "${c}" in
      ...
      5) menu_run_isolated_report "Add Adblock Source URL" adblock_url_add_menu ;;
      6) menu_run_isolated_report "Delete Adblock Source URL" adblock_url_delete_menu ;;
      7)
        hr
