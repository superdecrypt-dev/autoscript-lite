...
egress_menu() {
  while true; do
    title
    echo "5) Network Controls > Egress Mode & Balancer"
    hr
...
adblock_menu() {
  ...
  while true; do
    ...
    title
    echo "5) Network Controls > Adblock (Custom Geosite)"
    hr
...
warp_tier_switch_free() {
  title
  echo "5) Network Controls > WARP Controls > Switch ke WARP Free"
  hr
...
warp_tier_switch_plus() {
  ...
  title
  echo "5) Network Controls > WARP Controls > Switch ke WARP Plus"
  hr
...
warp_tier_reconnect_regenerate() {
  local target key
  title
  echo "5) Network Controls > WARP Controls > Reconnect/Regenerate"
  hr
...
warp_tier_menu() {
  while true; do
    title
    echo "5) Network Controls > WARP Controls > WARP Tier (Free/Plus)"
    hr
    warp_tier_show_status
    hr
    echo "  1) Show status"
    ...
    case "${c}" in
      1)
        title
        echo "5) Network Controls > WARP Controls > WARP Tier Status"
        hr
        warp_tier_show_status
        hr
        pause
...
warp_controls_menu() {
  while true; do
    title
    echo "5) Network Controls > WARP Controls"
    hr
...
domain_geosite_menu() {
  ...
  while true; do
    title
    echo "5) Network Controls > Domain/Geosite Routing (Direct List)"
    hr
...
dns_settings_menu() {
  while true; do
    title
    echo "5) Network Controls > DNS Settings"
    hr
...
dns_addons_menu() {
  while true; do
    title
    echo "5) Network Controls > DNS Add-ons"
    hr
    if [[ -f "${XRAY_DNS_CONF}" ]]; then
...
network_diagnostics_menu() {
  while true; do
    title
    echo "5) Network Controls > Diagnostics"
    hr
...
speedtest_run_now() {
  title
  echo "7) Speedtest > Run Speedtest"
  hr
