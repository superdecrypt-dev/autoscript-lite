...
cleanup_legacy_adblock_runtime() {
  local legacy_service="ssh-adblock-dns.service"
  local legacy_conf="/etc/autoscript/ssh-adblock/dnsmasq.conf"
  local legacy_sync_service="ssh-adblock-sync.service"
  local legacy_update_service="ssh-adblock-update.service"
  local legacy_update_timer="ssh-adblock-update.timer"
  local legacy_root="/etc/autoscript/ssh-adblock"
  local legacy_config="${legacy_root}/config.env"
  ...
  systemctl disable --now "${legacy_sync_service}" >/dev/null 2>&1 || true
  systemctl disable --now "${legacy_update_service}" >/dev/null 2>&1 || true
  systemctl disable --now "${legacy_update_timer}" >/dev/null 2>&1 || true
  if pgrep -af -- "${legacy_conf}" >/dev/null 2>&1; then
    pkill -f -- "${legacy_conf}" >/dev/null 2>&1 || true
    sleep 1
  fi
  for unit in \
    "/etc/systemd/system/${legacy_service}" \
    "/etc/systemd/system/${legacy_sync_service}" \
    "/etc/systemd/system/${legacy_update_service}" \
    "/etc/systemd/system/${legacy_update_timer}"; do
    if [[ -e "${unit}" ]]; then
      rm -f -- "${unit}"
      unit_changed=1
    fi
  done
  systemctl reset-failed "${legacy_service}" "${legacy_sync_service}" "${legacy_update_service}" "${legacy_update_timer}" >/dev/null 2>&1 || true
  if [[ "${unit_changed}" -eq 1 ]]; then
    systemctl daemon-reload >/dev/null 2>&1 || true
  fi
  if [[ -d "${legacy_root}" ]]; then
    install -d -m 0755 "${ADBLOCK_ROOT}"
    adblock_migrate_legacy_file "${legacy_root}/blocked.domains" "${ADBLOCK_BLOCKLIST_FILE}" || true
    adblock_migrate_legacy_file "${legacy_root}/source.urls" "${ADBLOCK_URLS_FILE}" || true
  ...
  if [[ "${migrated}" -eq 1 ]]; then
    ok "State legacy adblock dimigrasikan ke ${ADBLOCK_ROOT}."
  fi
}

install_adblock_runtime() {
  local dist_src="" dnsmasq_bin="" adblock_enabled="" adblock_dirty="" adblock_last_update=""
  ...
  adblock_enabled="${ADBLOCK_MIGRATED_ENABLED:-0}"
  adblock_dirty="${ADBLOCK_MIGRATED_DIRTY:-0}"
  adblock_last_update="${ADBLOCK_MIGRATED_LAST_UPDATE:-}"
  [[ "${adblock_enabled}" =~ ^(0|1|true|false|yes|no|on|off|y|n)$ ]] || adblock_enabled="0"
  [[ "${adblock_dirty}" =~ ^(0|1|true|false|yes|no|on|off|y|n)$ ]] || adblock_dirty="0"
  ...
  # Siapkan artefak kosong lebih dulu supaya dnsmasq bisa start pada bootstrap
  # awal, bahkan ketika blocklist masih kosong dan runtime sync belum pernah jalan.
  # Jika hasil migrasi legacy sudah ada, pertahankan artefak itu.
  if [[ ! -s "${ADBLOCK_RENDERED_FILE}" ]]; then
    cat > "${ADBLOCK_RENDERED_FILE}" <<'EOF'
# generated by adblock-sync
# do not edit directly
EOF
  fi
  if [[ ! -e "${ADBLOCK_MERGED_FILE}" ]]; then
    : > "${ADBLOCK_MERGED_FILE}"
  fi
  chmod 644 "${ADBLOCK_RENDERED_FILE}" "${ADBLOCK_MERGED_FILE}" >/dev/null 2>&1 || true
  chown root:root "${ADBLOCK_RENDERED_FILE}" "${ADBLOCK_MERGED_FILE}" 2>/dev/null || true
  ...
  render_setup_template_or_die \
    ...
    "ADBLOCK_DIRTY=${adblock_dirty}" \
    "ADBLOCK_LAST_UPDATE=${adblock_last_update}"
  ...
  if ! "${ADBLOCK_SYNC_BIN}" --apply >/dev/null 2>&1; then
    warn "Runtime adblock awal gagal diterapkan."
    systemctl status "${ADBLOCK_DNS_SERVICE}" --no-pager >&2 || true
    journalctl -u "${ADBLOCK_DNS_SERVICE}" -n 120 --no-pager >&2 || true
    die "Runtime adblock gagal diterapkan."
  fi

  if ! systemctl is-active --quiet "${ADBLOCK_DNS_SERVICE}"; then
    warn "Service ${ADBLOCK_DNS_SERVICE} tidak aktif setelah setup adblock."
    systemctl status "${ADBLOCK_DNS_SERVICE}" --no-pager >&2 || true
    journalctl -u "${ADBLOCK_DNS_SERVICE}" -n 120 --no-pager >&2 || true
    die "Runtime adblock tidak aktif setelah setup."
  fi
