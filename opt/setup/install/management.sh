...
EOF
  ...
  if ! "${ADBLOCK_SYNC_BIN}" --apply >/dev/null 2>&1; then
    warn "Runtime adblock awal gagal diterapkan; melanjutkan setup tanpa DNS Adblock aktif."
    systemctl status "${ADBLOCK_DNS_SERVICE}" --no-pager >&2 || true
    journalctl -u "${ADBLOCK_DNS_SERVICE}" -n 120 --no-pager >&2 || true
    systemctl disable --now "${ADBLOCK_DNS_SERVICE}" >/dev/null 2>&1 || true
    return 0
  fi

  if ! systemctl is-active --quiet "${ADBLOCK_DNS_SERVICE}"; then
    warn "Service ${ADBLOCK_DNS_SERVICE} tidak aktif setelah setup adblock; melanjutkan setup tanpa DNS Adblock aktif."
    systemctl status "${ADBLOCK_DNS_SERVICE}" --no-pager >&2 || true
    journalctl -u "${ADBLOCK_DNS_SERVICE}" -n 120 --no-pager >&2 || true
    systemctl disable --now "${ADBLOCK_DNS_SERVICE}" >/dev/null 2>&1 || true
    return 0
