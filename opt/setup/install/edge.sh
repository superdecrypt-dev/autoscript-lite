...
EDGE_XRAY_DIRECT_BACKEND="${EDGE_XRAY_DIRECT_BACKEND:-${EDGE_SSH_CLASSIC_BACKEND:-${EDGE_NGINX_HTTP_BACKEND:-127.0.0.1:18080}}}"
EDGE_XRAY_TLS_BACKEND="${EDGE_XRAY_TLS_BACKEND:-${EDGE_SSH_TLS_BACKEND:-${EDGE_NGINX_TLS_BACKEND:-127.0.0.1:18443}}}"
EDGE_XRAY_WS_BACKEND="${EDGE_XRAY_WS_BACKEND:-${EDGE_SSH_WS_BACKEND:-${EDGE_NGINX_HTTP_BACKEND:-127.0.0.1:18080}}}"
EDGE_XRAY_FALLBACK_BACKEND="${EDGE_XRAY_FALLBACK_BACKEND:-${EDGE_OPENVPN_TCP_BACKEND:-${EDGE_NGINX_TLS_BACKEND:-127.0.0.1:18443}}}"
EDGE_XRAY_QUOTA_ROOT="${EDGE_XRAY_QUOTA_ROOT:-${EDGE_SSH_QUOTA_ROOT:-/opt/quota/xray}}"
EDGE_XRAY_RUNTIME_UNIT="${EDGE_XRAY_RUNTIME_UNIT:-${EDGE_SSH_DROPBEAR_UNIT:-xray}}"
EDGE_XRAY_QAC_ENFORCER="${EDGE_XRAY_QAC_ENFORCER:-${EDGE_SSH_QAC_ENFORCER:-/usr/local/bin/true}}"
EDGE_XRAY_MANAGE_BIN="${EDGE_XRAY_MANAGE_BIN:-${EDGE_SSH_MANAGE_BIN:-/usr/local/bin/manage}}"
...
EDGE_XRAY_SESSION_ROOT="${EDGE_XRAY_SESSION_ROOT:-${EDGE_SSH_SESSION_ROOT:-/run/autoscript/xray-edge-sessions}}"
EDGE_XRAY_SESSION_HEARTBEAT_SEC="${EDGE_XRAY_SESSION_HEARTBEAT_SEC:-${EDGE_SSH_SESSION_HEARTBEAT_SEC:-15}}"
...
load_persisted_edge_runtime_env() {
  ...
  while IFS='=' read -r key value; do
    case "${key}" in
      EDGE_PROVIDER|EDGE_ACTIVATE_RUNTIME|EDGE_PUBLIC_HTTP_PORT|EDGE_PUBLIC_TLS_PORT|EDGE_PUBLIC_HTTP_PORTS|EDGE_PUBLIC_TLS_PORTS|EDGE_METRICS_ENABLED|EDGE_METRICS_LISTEN|EDGE_NGINX_HTTP_BACKEND|EDGE_NGINX_TLS_BACKEND|EDGE_NGINX_STREAM_CONF|EDGE_XRAY_DIRECT_BACKEND|EDGE_XRAY_TLS_BACKEND|EDGE_XRAY_WS_BACKEND|EDGE_XRAY_FALLBACK_BACKEND|EDGE_XRAY_QUOTA_ROOT|EDGE_XRAY_VLESS_RAW_BACKEND|EDGE_XRAY_TROJAN_RAW_BACKEND|EDGE_XRAY_RUNTIME_UNIT|EDGE_XRAY_QAC_ENFORCER|EDGE_XRAY_MANAGE_BIN|EDGE_HTTP_DETECT_TIMEOUT_MS|EDGE_CLASSIC_TLS_ON_80|EDGE_TLS_CERT_FILE|EDGE_TLS_KEY_FILE|EDGE_TLS_HANDSHAKE_TIMEOUT_MS|EDGE_MAX_CONNS|EDGE_MAX_CONNS_PER_IP|EDGE_ACCEPT_RATE_LIMIT_PER_IP|EDGE_ACCEPT_RATE_WINDOW_SEC|EDGE_ABUSE_COOLDOWN_TRIGGER_REJECTS|EDGE_ABUSE_COOLDOWN_WINDOW_SEC|EDGE_ABUSE_COOLDOWN_SEC|EDGE_XRAY_SESSION_ROOT|EDGE_XRAY_SESSION_HEARTBEAT_SEC|EDGE_ACCEPT_PROXY_PROTOCOL|EDGE_TRUSTED_PROXY_CIDRS|EDGE_SNI_ROUTES|EDGE_SNI_PASSTHROUGH|EDGE_SSH_CLASSIC_BACKEND|EDGE_SSH_TLS_BACKEND|EDGE_SSH_WS_BACKEND|EDGE_OPENVPN_TCP_BACKEND|EDGE_SSH_QUOTA_ROOT|EDGE_SSH_DROPBEAR_UNIT|EDGE_SSH_QAC_ENFORCER|EDGE_SSH_MANAGE_BIN|EDGE_SSH_SESSION_ROOT|EDGE_SSH_SESSION_HEARTBEAT_SEC)
        value="${value%$'\r'}"
...
write_edge_runtime_env() {
  ...
  render_setup_template_or_die \
    ...
    "EDGE_XRAY_DIRECT_BACKEND=${EDGE_XRAY_DIRECT_BACKEND}" \
    "EDGE_XRAY_TLS_BACKEND=${EDGE_XRAY_TLS_BACKEND}" \
    "EDGE_XRAY_WS_BACKEND=${EDGE_XRAY_WS_BACKEND}" \
    "EDGE_XRAY_FALLBACK_BACKEND=${EDGE_XRAY_FALLBACK_BACKEND}" \
    "EDGE_XRAY_QUOTA_ROOT=${EDGE_XRAY_QUOTA_ROOT}" \
    "EDGE_XRAY_VLESS_RAW_BACKEND=${vless_raw_backend}" \
    "EDGE_XRAY_TROJAN_RAW_BACKEND=${trojan_raw_backend}" \
    "EDGE_XRAY_RUNTIME_UNIT=${EDGE_XRAY_RUNTIME_UNIT}" \
    "EDGE_XRAY_QAC_ENFORCER=${EDGE_XRAY_QAC_ENFORCER}" \
    "EDGE_XRAY_MANAGE_BIN=${EDGE_XRAY_MANAGE_BIN}" \
    ...
    "EDGE_ABUSE_COOLDOWN_WINDOW_SEC=${EDGE_ABUSE_COOLDOWN_WINDOW_SEC:-30}" \
    "EDGE_ABUSE_COOLDOWN_SEC=${EDGE_ABUSE_COOLDOWN_SEC:-120}" \
    "EDGE_XRAY_SESSION_ROOT=${EDGE_XRAY_SESSION_ROOT}" \
    "EDGE_XRAY_SESSION_HEARTBEAT_SEC=${EDGE_XRAY_SESSION_HEARTBEAT_SEC}" \
    "EDGE_ACCEPT_PROXY_PROTOCOL=${EDGE_ACCEPT_PROXY_PROTOCOL:-false}" \
    "EDGE_TRUSTED_PROXY_CIDRS=${EDGE_TRUSTED_PROXY_CIDRS:-127.0.0.1/32,::1/128}" \
    "EDGE_SNI_ROUTES=${EDGE_SNI_ROUTES:-}" \
    "EDGE_SNI_PASSTHROUGH=${EDGE_SNI_PASSTHROUGH:-}"
