...
EDGE_SSH_QUOTA_ROOT="${EDGE_SSH_QUOTA_ROOT:-/opt/quota/ssh-ovpn}"
EDGE_SSH_DROPBEAR_UNIT="${EDGE_SSH_DROPBEAR_UNIT:-sshws-dropbear}"
EDGE_SSH_QAC_ENFORCER="${EDGE_SSH_QAC_ENFORCER:-/usr/local/bin/sshws-qac-enforcer}"
...
EDGE_SSH_SESSION_ROOT="${EDGE_SSH_SESSION_ROOT:-/run/autoscript/sshws-sessions}"
...
EDGE_OVPN_TCP_BACKEND="${EDGE_OVPN_TCP_BACKEND:-}"
EDGE_OVPN_ENABLE_TCP="${EDGE_OVPN_ENABLE_TCP:-}"
EDGE_OVPN_ENABLE_SSL="${EDGE_OVPN_ENABLE_SSL:-}"

edge_ovpn_tcp_backend_value() {
  local value="${EDGE_OVPN_TCP_BACKEND:-}"
  if [[ -z "${value}" ]]; then
    value="${OVPN_TCP_BIND:-127.0.0.1}:${OVPN_TCP_PORT:-21194}"
  fi
  printf '%s\n' "${value}"
}

edge_ovpn_enable_tcp_value() {
  local value="${EDGE_OVPN_ENABLE_TCP:-}"
  if [[ -z "${value}" ]]; then
    value="${OVPN_ENABLE_TCP:-false}"
  fi
  printf '%s\n' "${value}"
}

edge_ovpn_enable_ssl_value() {
  local value="${EDGE_OVPN_ENABLE_SSL:-}"
  if [[ -z "${value}" ]]; then
    value="${OVPN_ENABLE_SSL:-false}"
  fi
  printf '%s\n' "${value}"
}
...
load_persisted_edge_runtime_env() {
  ...
  while IFS='=' read -r key value; do
    case "${key}" in
      EDGE_PROVIDER|EDGE_ACTIVATE_RUNTIME|EDGE_PUBLIC_HTTP_PORT|EDGE_PUBLIC_TLS_PORT|EDGE_METRICS_ENABLED|EDGE_METRICS_LISTEN|EDGE_NGINX_HTTP_BACKEND|EDGE_NGINX_TLS_BACKEND|EDGE_NGINX_STREAM_CONF|EDGE_SSH_CLASSIC_BACKEND|EDGE_SSH_TLS_BACKEND|EDGE_SSH_QUOTA_ROOT|EDGE_SSH_DROPBEAR_UNIT|EDGE_SSH_QAC_ENFORCER|EDGE_HTTP_DETECT_TIMEOUT_MS|EDGE_CLASSIC_TLS_ON_80|EDGE_TLS_CERT_FILE|EDGE_TLS_KEY_FILE|EDGE_TLS_HANDSHAKE_TIMEOUT_MS|EDGE_MAX_CONNS|EDGE_MAX_CONNS_PER_IP|EDGE_ACCEPT_RATE_LIMIT_PER_IP|EDGE_ACCEPT_RATE_WINDOW_SEC|EDGE_ABUSE_COOLDOWN_TRIGGER_REJECTS|EDGE_ABUSE_COOLDOWN_WINDOW_SEC|EDGE_ABUSE_COOLDOWN_SEC|EDGE_SSH_SESSION_ROOT|EDGE_SSH_SESSION_HEARTBEAT_SEC|EDGE_ACCEPT_PROXY_PROTOCOL|EDGE_TRUSTED_PROXY_CIDRS|EDGE_OVPN_TCP_BACKEND|EDGE_OVPN_ENABLE_TCP|EDGE_OVPN_ENABLE_SSL)
        value="${value%$'\r'}"
...
write_edge_runtime_env() {
  ...
  render_setup_template_or_die \
    ...
    "EDGE_ACCEPT_PROXY_PROTOCOL=${EDGE_ACCEPT_PROXY_PROTOCOL:-false}" \
    "EDGE_TRUSTED_PROXY_CIDRS=${EDGE_TRUSTED_PROXY_CIDRS:-127.0.0.1/32,::1/128}" \
    "EDGE_OVPN_TCP_BACKEND=$(edge_ovpn_tcp_backend_value)" \
    "EDGE_OVPN_ENABLE_TCP=$(edge_ovpn_enable_tcp_value)" \
    "EDGE_OVPN_ENABLE_SSL=$(edge_ovpn_enable_ssl_value)"
}
