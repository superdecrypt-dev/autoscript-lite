...
Repositori ini memiliki area root untuk skrip operasional server: `setup.sh` (provisioning awal), `manage.sh` (menu harian), `run.sh` (bootstrap installer), dan `install-telegram-bot.sh`. Source modular `manage.sh` berada di `opt/manage/` (sinkron ke `/opt/manage` di VPS).  
...
- `bash -n setup.sh manage.sh run.sh install-telegram-bot.sh`: validasi syntax skrip shell.
...
- `sudo bash run.sh`: instalasi cepat (pasang `manage` dan `install-telegram-bot` ke `/usr/local/bin` lalu jalankan setup).
...
- `TESTING_PLAYBOOK.md`: SOP pengujian lengkap untuk shell script + bot Telegram (preflight, smoke, negative, integration, gate).
...
Gunakan Bash strict mode (`set -euo pipefail`) dan pola defensif yang sudah ada (`ok`, `warn`, `die`). Indentasi utama 2 spasi untuk shell. Nama fungsi `snake_case`, konstanta/env `UPPER_SNAKE_CASE`, nama skrip `kebab-case.sh`. Untuk Python bot, gunakan nama modul yang deskriptif per domain menu (`menu_1_status`, `menu_8_maintenance`, dst).

## Panduan Testing
Minimum sebelum merge: syntax check + lint shell + smoke check layanan terkait. Untuk perubahan runtime Xray, verifikasi `systemctl status xray xray-expired xray-quota xray-limit-ip xray-speed --no-pager` dan `xray run -test -confdir /usr/local/etc/xray/conf.d`. Untuk bot Telegram, uji `backend-py` health endpoint ber-auth secret dan flow `/menu` + `/cleanup`.
...
Jangan commit token/secret/key. Simpan rahasia pada env file runtime dan gunakan masking saat ditampilkan. Semua skrip diasumsikan berjalan sebagai root; selalu uji dulu di VPS non-produksi sebelum rollout ke produksi.
...
- Bot Telegram diposisikan sebagai pelengkap CLI `manage.sh`, bukan pengganti penuh alur CLI.
...
- Fokus sprint terbaru: refresh UX bot Telegram + sinkronisasi dokumentasi + revalidasi live `run.sh`.
